{
  ...
  # mcc-env = (pkgs.callPackage miniCompileCommands {}).wrap pkgs.gcc14Stdenv;
  mcc-env = (pkgs.callPackage miniCompileCommands {}).wrap pkgs.llvmPackages_18.stdenv;
  ...
  package = mcc-env.mkDerivation (self: {
    ...
    cmakeFlags = [
      ...
      # "DCMAKE_CXX_FLAGS=-isystem ${pkgs.llvmPackages_18.libcxx}/include/c++/v1"
  ...
  shell = (pkgs.mkShell.override {stdenv = mcc-env;}) {
    ...
    shellHook = ''
      ...
      alias ll="ls -l"
      cowsay "Welcome to the '${package.name}' dev environment!"
    '';
  };
in
  package
